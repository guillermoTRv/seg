<?php 
    ...
    $mes_actual = date("n");
    if($tipo == "derecha"){$resta = $mes_get - $mes_actual +1;$month=$mes_get+1;}
	if($tipo == "izquierda"){$resta_left = $mes_get - $mes_actual;$month=$mes_get-1;}    
    if ($resta == 3) {
    	echo "si";
    }
    else{	
		    ...
		    <div class="calendario" data="<?php echo $month ?>">  
		    	<?php
		            ...
		            $num_primer_dia = num_dia($primer_dia);
		   			...
		   			<center>
					<table>
					<caption><h4 style="font-weight: bold">
						<?php if($resta_left!=1){
							?>
								<span class="icon-circle-left mover" data-idsala="<?php echo $id_sala ?>" data-namesala = "<?php echo $name_sala ?>" data-mes="<?php echo $month ?>" data-tipo='izquierda'></span>
							<?php
						} ?> 
						Mes <?php echo mes($month) ?> <span data-idsala="<?php echo $id_sala ?>" data-namesala = "<?php echo $name_sala ?>" data-mes="<?php echo $month ?>" data-tipo='derecha' class='icon-circle-right mover'></span></h4></caption>
					<tr>
		                  <th>Lun</th>
		                  <th>Mar</th>
		                  <th>Mie</th>
