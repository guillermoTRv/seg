<?php 
    ...
    $tiempo_inicio_blue = $hrs_inicio+($min_inicio/60);
	$tiempo_fin_blue    = $hrs_fin+($min_fin/60);
	$consulta_hay_uno = consulta_val("SELECT null FROM reservas_juntas WHERE dia='$dia' and id_usuario='$id_usuario' and (tiempo_inicio >= '$tiempo_inicio_blue' and tiempo_inicio <= '$tiempo_fin_blue')");
	
	$consulta_hay_two = consulta_val("SELECT null FROM reservas_juntas WHERE dia='$dia' and id_usuario='$id_usuario' and (tiempo_fin >= '$tiempo_inicio_blue' and tiempo_fin <= '$tiempo_fin_blue')");

	$consulta_hay_tre = consulta_val("SELECT null FROM reservas_juntas WHERE dia='$dia' and id_usuario='$id_usuario' and (tiempo_inicio >= '$tiempo_inicio_blue' and tiempo_fin <= '$tiempo_fin_blue')");
	
	$consulta_hay_fou = consulta_val("SELECT null FROM reservas_juntas WHERE dia='$dia' and id_usuario='$id_usuario' and (tiempo_inicio <= '$tiempo_inicio_blue' and tiempo_inicio >= '$tiempo_fin_blue')");
	if ($consulta_hay_uno == 0 && $consulta_hay_two == 0 && $consulta_hay_tre == 0 && $consulta_hay_fou == 0) {
		...
		if ($consulta_disponibilidad == 0 ) {
			...
			<div style="height:20px;width:100%;background-color:">
				...
				?>
				<div class="disponible" <?php echo $datas ?> style="display:inline-block;height:30px;width:100%;background-color:brown;margin-right:-4px;padding-left: 2px;padding-top: 1px">
			...
			<div style="height:20px;width:100%;">
				...
				if ($hrs_inicio < $rango_redon_for && $tiempo_fin <= $rango_redon_for) {
				?>
				<div class="disponible" <?php echo $datas ?> style="display:inline-block;height:30px;width:<?php echo $width_ultimo ?>%;background-color:brown;margin-right:-4px;padding-left:2px;padding-top: 1px">
				  	<p class="p_hora_mens"><span class='icon-checkmark'></span></p>
				</div>
				<?php
				}
