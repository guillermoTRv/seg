	<?php 
		if ($hrs_inicio < $rango_redon_for) {
			$minutos_a_horasIn = $min_inicio/60; 
			$oper_b          = ((($hrs_inicio+$minutos_a_horasIn)-$hora_inicio_turno)/$rango_redon)*100;

			$minutos_a_horasFn = $min_fin/60;
			$oper_c = ((($hrs_fin+$minutos_a_horasFn)-($hrs_inicio+$minutos_a_horasIn))/$rango_redon)*100;

			$oper_e = ($oper_b+$oper_c)-100;
			$oper_f = 100-$oper_b;
	}
	if ($oper_b != 0) {
	?>
	<div class='disponible' <?php echo $datas ?> style="display:inline-block;height:30px;width:<?php echo $oper_b ?>%;background-color:brown;margin-right:-5px;padding-left: 2px;padding-top: 1px">
		  	<p class="p_hora_mens"><span class='icon-checkmark'></span></p>
	</div>
	<?php
	}
	?>
	<div class='disponible' <?php echo $datas_con ?> style="display:inline-block;height:30px;width:<?php echo $oper_f ?>%;background-color:rgb(129, 218, 245);margin-right:-5px;padding-left: 2px;padding-top: 1px">
		  	<p class="p_hora_mens"><span class='icon-user-check'></span></p>
	</div>
	