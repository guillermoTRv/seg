<?php 
	...
	if ($otra_jr == "otra_jornada") {
		$validar_jornada_uno = consulta_val("SELECT null FROM jornadas_trabajo 
													 WHERE fecha_entrada = '$fecha_entrada_post' and identificador = '$identificador'");
		if ($validar_jornada_uno == 0) {
			...
			?><script>window.location.href = "personal.php?pr=hr&val=<?php echo $identificador ?>";</script> <?php
		}

		if ($validar_jornada_uno > 0 && $validar_jornada_uno < 3) {
			        
			        $validar_jornada_dos = consulta_tx("SELECT hora_salida FROM jornadas_trabajo WHERE fecha_entrada = '$fecha_entrada_post'and identificador = '$identificador' order by id_jornada desc","hora_salida");
